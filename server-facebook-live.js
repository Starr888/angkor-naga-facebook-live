...
- ឆ្លើយខ្លីណាស់។ ឆ្លើយតែ ១ ប្រយោគ ឬ ២ ប្រយោគខ្លីៗ ប្រហែល ៨ ទៅ ១៨ ពាក្យប៉ុណ្ណោះ។
- បើអ្នកមើលនិយាយ hello, hi, សួស្តី ឬសួរខ្លីៗ ត្រូវឆ្លើយជាការស្វាគមន៍ខ្លីៗ មិនត្រូវពន្យល់ប្រវត្តិសាស្ត្រវែង។
- ពន្យល់ប្រវត្តិសាស្ត្រវែង តែពេលអ្នកមើលសុំថា “ពន្យល់”, “ប្រាប់លម្អិត”, “tell more”, ឬ “ប្រវត្តិ”។
- កុំចែចង់។ កុំប្រើពាក្យ adult, sexy, baby, girlfriend, love។
- កុំនិយាយថាខ្លួនជាអេអាយ bot model server ឬ assistant។
    ...
    try {
      ...
      if (msg.type === 'control_comment') {
        ...
        if (room.character === 'zhou-daguan') {
          const shortGreeting =
            /^(hi|hello|hey|សួស្តី|ជំរាបសួរ|ok|okay|👍|❤️|🙏)\b/i.test(text.trim());

          if (shortGreeting) {
            promptText =
              `អ្នកមើលនិយាយខ្លីថា: "${text}". ` +
              `ឆ្លើយជាភាសាខ្មែរ ១ ប្រយោគខ្លីបំផុត ដូចជាការស្វាគមន៍។ ` +
              `កុំចាប់ផ្តើមពន្យល់ប្រវត្តិសាស្ត្រវែង។ កុំនិយាយថាខ្លួនជាអេអាយ។`;
          } else {
            promptText =
              `មតិអ្នកមើល: "${text}". ` +
              `តួអង្គដែលត្រូវឆ្លើយ: ជីវ តាក្វាន់ / Zhou Daguan. ` +
              `ឆ្លើយជាភាសាខ្មែរ ខ្លីណាស់ ត្រឹម ១ ទៅ ២ ប្រយោគខ្លីៗ ប្រហែល ៨ ទៅ ១៨ ពាក្យ។ ` +
              `បើសំណួរមិនបានសុំលម្អិត កុំឆ្លើយវែង។ ` +
              `ផ្តោតតែអង្គរ កំណត់ត្រា The Customs of Cambodia និងប្រវត្តិសាស្ត្រខ្មែរ។ ` +
